...
class RelayTest(unittest.TestCase):
    ...
    def test_complete_snapshot_parts_inbox_and_replay(self):
        ...
        parts=self.store.db.execute('SELECT o.text FROM permission_parts p JOIN outbox o ON o.id=p.event_id ORDER BY p.part').fetchall()
        rebuilt=''.join(row[0].split('\n',2)[2] for row in parts)
    ...
    def test_old_prompt_or_missing_native_does_not_route(self):
        ...
        self.assertIsNone(observe(self.store,dict(self.run,native_session_id=None),self.info))
    ...
    def test_delivery_identity_binding_and_rejection(self):
        observe(self.store,self.run,self.info)
